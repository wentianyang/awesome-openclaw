...
本列表中的项目经过筛选，但**未经审计**。使用风险自担。安装前请务必检查任何技能或插件的源代码。

请 **切勿** 分享您的 `OPENAI_API_KEY` 或任何其他敏感凭证。
...
- **[DingTalk Connector](https://github.com/DingTalk-Real-AI/dingtalk-openclaw-connector)** - 钉钉官方连接器，支持 AI 卡片流式响应，实现企业级深度集成。

## ⚙️ 编排与框架 (Orchestration & Frameworks)

- **[Antfarm (Snarktank)](https://github.com/snarktank/antfarm)** - 为 OpenClaw 打造的多代理编排框架，通过 YAML 定义开发团队与审计工作流。
...
- **[MimiClaw (memovai)](https://github.com/memovai/mimiclaw)** - 专为 ESP32-S3 微控制器优化的高性能 OpenClaw 变体，支持在 5 美元的芯片上运行。
- **[OpenClawInstaller (miaoxworld)](https://github.com/miaoxworld/OpenClawInstaller)** - OpenClaw 的一键全自动部署与配置工具，支持多平台环境。
- **[Tokscale (junhoyeo)](https://github.com/junhoyeo/tokscale)** - 专为 OpenClaw.ai 优化的 Token 使用量追踪工具，支持可视化排行。
- **[Manifest (mnfst)](https://github.com/mnfst/manifest)** - 为 OpenClaw 打造的智能 LLM 路由工具，最高可降低 70% 的 Token 成本并提供实时监控。
...
- **[Crabwalk (luccast)](https://github.com/luccast/crabwalk)** - OpenClaw 智能体的实时观察哨，提供思考与执行链的活性节点图展示。

## 🧩 社区技能 (Community Skills)

- **[XiaohongshuSkills](https://github.com/white0dew/XiaohongshuSkills)** - 支持小红书自动发布、自动评论、自动检索的强大技能。支持 OpenClaw。
- **[X Research Skill (rohunvora)](https://github.com/rohunvora/x-research-skill)** - 专为 X (Twitter) 打造的调研工具，支持智能搜索、推特线程追踪以及自动简报。
...
- **[ClawWork (HKUDS)](https://github.com/HKUDS/ClawWork)** - 基于 nanobot（极简版 OpenClaw）的 AI 同事框架，引入了经济责任制和专业的任务评估机制。
